...
export function getSupportedPostTypesIntersection(platforms: Array<SocialMedia | null | undefined>): PostType[] {
...
export function getAggregatedTagsConfig(platforms: Array<SocialMedia | null | undefined>): {
  ...
  const tags = uniquePlatforms
    .map((p) => getPlatformConfig(p)?.tags)
    .filter(Boolean);
