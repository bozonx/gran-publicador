...
import { formatTagsCsv, normalizeTags } from '~/utils/tags'
...
export function aggregateSelectedItemsToPublicationOrThrow(
  ...
  const tags = formatTagsCsv(normalizePublicationTags(allTags, limits.MAX_TAGS_COUNT))
  ...
  if (tags.length > limits.MAX_TAGS_LENGTH) {
