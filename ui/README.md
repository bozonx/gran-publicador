...
Приложение **осознанно работает в режиме SPA** (`ssr: false`).

Причины:
- Основной сценарий использования — Telegram Mini App / клиентское приложение.
- SEO и серверный рендеринг не являются целевыми требованиями.

Следствие:
- Все интеграции, завязанные на `window` / Telegram SDK, должны оставаться client-only.
...
Юнит и Nuxt-тесты запускаются через Vitest, а E2E-тесты — через Playwright:

```bash
# Установка браузеров (нужно запустить один раз)
...
pnpm test:nuxt    # Nuxt интеграционные тесты (Vitest)
pnpm test:e2e     # E2E тесты (Playwright)

# Другие команды
pnpm test:watch   # Запуск Vitest в режиме watch
pnpm test:cov     # Отчет о покрытии (Vitest)
```
