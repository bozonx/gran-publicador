...
- Поиск по тегам, источнику, дате добавления.
...
- Пользователь выбирает 1..N элементов из библиотеки и:
  ...
  - копирует/объединяет тексты в `Publication.content` или создаёт новый элемент библиотеки?
...
- **Личная библиотека**: отдельная страница/раздел на уровне пользователя (без привязки к проекту).
- **Проектная библиотека**: отдельная страница/раздел внутри страницы проекта, общая для участников проекта и настроенная под контекст проекта.
- **Изоляция данных**: элементы личной библиотеки не видны участникам проекта, если они явно не добавлены в проектную библиотеку (публикация в проект и библиотека проекта не должны считаться одним и тем же по умолчанию).
...
### 10.2 UX-набросок
- Экран «Библиотека» (общий паттерн для личной и проектной):
  - поиск
  - фильтры
...
- **Личный раздел**: «Библиотека» в пользовательском контексте показывает только personal-контент.
- **Раздел проекта**: «Библиотека контента» на странице проекта показывает только project-контент и может иметь проектные фильтры/пресеты.
...
- Согласование модели контента: возможно часть «сырья» логичнее хранить в библиотеке, а в публикацию копировать только нужное.
